...
class HiltCapabilityTest {
    @TempDir
    Path projectDir

    @Test
    void hiltSugarEnablesKspPluginAndDependenciesForAndroidApplication() {
...
simpledsl {
    androidApplication {
        namespace = 'example.hilt.app'
    }
    hilt()
}

assert pluginManager.hasPlugin('com.google.devtools.ksp')
assert pluginManager.hasPlugin('com.google.dagger.hilt.android')
...
assert configurations.getByName('implementation').dependencies.any {
    it.group == 'com.google.dagger' && it.name == 'hilt-android' && it.version == '2.60.1'
}
assert configurations.getByName('ksp').dependencies.any {
    it.group == 'com.google.dagger' && it.name == 'hilt-compiler' && it.version == '2.60.1'
}

def model = extensions.getByName('simpledslModuleModel')
assert model.capabilities.get().contains('ksp')
assert model.capabilities.get().contains('hilt')
''')
    ...
    @Test
    void genericHiltCapabilityUsesSameContractForAndroidLibrary() {
...
simpledsl {
    androidLibrary {
        namespace = 'example.hilt.feature'
    }
    capability('hilt')
}
...
''')
    ...
    @Test
    void hiltSugarUsesNormalModuleTypeDiagnostic() {
...
''')
        ...
        assertOutputContains(result, 'Capability: hilt')
...
rootProject.name = 'android-hilt-consumer'
include '${moduleName}'
""".stripIndent())
    }
